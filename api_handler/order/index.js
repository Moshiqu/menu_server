...
exports.orderConfirmHandler = (req, res) => {
  ...
  // 新建订单
  const ordersSql = `INSERT INTO orders (id, owner_id, consumer_id, order_price) VALUES ('${order_id}', ${ownerId}, ${consumerId}, ${
    orderPrice || 0
  })`
...
exports.getClassicOrderHandler = (req, res) => {
  ...
  db.query(
    `SELECT order_status, order_price, created_time, id FROM orders WHERE ${
      classic == 1 ? 'owner_id' : 'consumer_id'
    } = ? AND order_status IN (1,2,3,4) AND is_active = 1 ORDER BY created_time DESC`,
    userId,
    (err, orders) => {
